use bod_models::{
    schemas::config::company::{models::company_with_id::CompanyWithId, company::Company},
...
lazy_static! {
    pub static ref COMPANY_REPOSITORY: Arc<Mutex<Option<CompanyRepository>>> = Arc::new(Mutex::new(None));
}
    ...
    async fn set_repository(
        ...
        let repository_option = {
            ...
            if value.lock().unwrap().is_none() {
                let obj_repository = CompanyRepository::new(repository)?;
